...
import { resolveMediaUrl, BACKEND_URL } from "@/lib/utils";
...
export function PmbLandingPage({ onOpenRegister, onOpenLogin, onAuth, branding, onBackToSiakad }) {
  ...
  const [brandingState, setBrandingState] = useState(branding || { name: "POLITEKNIK SCI" });
  ...
  useEffect(() => {
    if (branding && (branding.campus_logo_url || branding.logo_url || branding.campus_name)) {
      setBrandingState(branding);
    }
  }, [branding]);

  useEffect(() => {
    // Fetch SIAKAD public branding settings
  ...
  const faqsList = settings?.landing_faqs?.length > 0 ? settings.landing_faqs : [
    ...
    { q: "Apakah tersedia pilihan kuliah Online (Daring Penuh) dan Kelas Khusus Eksekutif?", a: "Ya! Untuk Kelas Reguler tersedia opsi mode Online (Daring Penuh) dan Offline (Tatap Muka), serta Kelas Weekend Online. Untuk Kelas Khusus Offline, perkuliahan diselenggarakan dengan kurikulum khusus eksekutif dan pembelajaran tatap muka di kampus." },
  ...
  const campusLogoUrl = settings?.landing_logo_url || brandingState?.pmb_logo_url || brandingState?.campus_logo_url || brandingState?.logo_url;
  const campusDisplayName = brandingState?.campus_code?.trim() || brandingState?.campus_name?.trim() || brandingState?.name?.trim() || "POLITEKNIK SCI";
  const campusFullName = brandingState?.campus_name?.trim() || brandingState?.name?.trim() || campusDisplayName;
  ...
  return (
    <div className="min-h-screen sci-landing-bg text-slate-100 font-sans selection:bg-sky-500 selection:text-white relative">
      ...
      {/* 13. Floating WhatsApp Instant Consultation Widget */}
      <PmbWhatsAppFloatingWidget settings={settings} branding={brandingState} />
    </div>
  );
}
